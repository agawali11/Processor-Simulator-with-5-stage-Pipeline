...
static bool registerValidArray[16]={true};
 ...
 * Note: You are free to edit this function according to your implementation
 */
void checkForwarding(APEX_CPU *cpu){
  //memory
  
      if(cpu->writeback.has_insn && cpu->decode.rs1==cpu->writeback.rd)
        {
          cpu->decode.rs1_value = cpu->writeback.result_buffer;
        }
      if(cpu->writeback.has_insn && cpu->decode.rs2==cpu->writeback.rd)
        {
          
          cpu->decode.rs2_value = cpu->writeback.result_buffer;
          
        }
        if(cpu->writeback.has_insn && cpu->decode.rs3==cpu->writeback.rd)
        {
          cpu->decode.rs3_value = cpu->writeback.result_buffer;
        }
        // execute
        if(cpu->memory.has_insn && cpu->decode.rs1==cpu->memory.rd)
        {
              cpu->decode.rs1_value = cpu->execute.result_buffer;
        }
      if(cpu->memory.has_insn && cpu->decode.rs2==cpu->memory.rd)
        {
          cpu->decode.rs2_value = cpu->memory.result_buffer;
        }
        if(cpu->memory.has_insn && cpu->decode.rs3==cpu->memory.rd)
        {
    ...
    if (cpu->decode.has_insn)
    {
        ...
        switch (cpu->decode.opcode)
        {
            ...
            case OPCODE_XOR:
            {
                ...
                checkForwarding(cpu);
                registerValidArray[cpu->decode.rd]=false;
                break;
            }
...
APEX_execute(APEX_CPU *cpu)
{
    if (cpu->execute.has_insn)
    {
        ...
        switch (cpu->execute.opcode)
        {
            ...
            {
                cpu->execute.result_buffer
                    = cpu->execute.rs1_value +(-1) * cpu->execute.imm;
    ...
    if (cpu->writeback.has_insn)
    {
        ...
        switch (cpu->writeback.opcode)
        {
            ...
            case OPCODE_XOR:
            {
                ...
                registerValidArray[cpu->writeback.rd]=true;
                break;
            }
 ...
 */
APEX_CPU *
APEX_cpu_init(const char *filename, const char *command)
{
    int i;
    APEX_CPU *cpu;
...
void display(APEX_CPU *cpu)
{
  ...
  for(i=0;i<16;i++)
  {
    ...
    if(registerValidArray[i])
    {
      printf("\n| REG[%d] | Value = %d | Status = VALID |\n",i,cpu->regs[i]);  
    }
    else
    {
      printf("\n| REG[%d] | Value = %d | Status = INVALID |\n",i,cpu->regs[i]);
    }
 ...
 */
void
APEX_cpu_run(APEX_CPU *cpu,const int counter)
{
